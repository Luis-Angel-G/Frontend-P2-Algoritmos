...
function FriendsManager() {
  ...
  useEffect(() => {
    ...
    setUser(storedUser)
    setAmigos(storedUser.amigos || [])
  }, [navigate])

  const handleAgregar = async (e) => {
    e.preventDefault()
    if (!nuevoAmigo || amigos.includes(nuevoAmigo) || nuevoAmigo === user.correo) {
      setMsg("Correo inválido o ya es tu amigo.")
      return
    }
    setLoading(true)
    setMsg("")
    try {
      ...
      if (response.ok) {
        ...
        const updatedUser = { ...user, amigos: [...amigos, nuevoAmigo] }
